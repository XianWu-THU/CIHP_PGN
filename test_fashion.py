...
def main():
    ...
    with tf.name_scope("create_inputs"):
        reader = BaseReader(DATA_DIR, LIST_PATH, None, False, False, coord)
        image = reader.image
    ...
    # evaluate prosessing
    parsing_dir = './output/fashion/parsing/' + PHASE
    if not os.path.exists(parsing_dir):
        os.makedirs(parsing_dir)
    visual_dir = './output/fashion/visual/' + PHASE
    if not os.path.exists(visual_dir):
        os.makedirs(visual_dir)
    edge_dir = './output/fashion/edge/' + PHASE
    ...
    for step in range(NUM_STEPS):
        ...
        parsing_im.save('{}/{}.png'.format(visual_dir, img_id))
